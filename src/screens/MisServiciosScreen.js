// src/screens/MisServiciosScreen.js
...
// Bump de reputación usando increment(1). Compatible con tus reglas actuales.
...
async function bumpReputation(db, targetUid, value /*, requestId */) {
  ...
  try {
    ...
    // Si el doc no existe, crea stub con UN solo campo (regla canCreateReputationStub)
    if (e?.code === "not-found") {
      if (value === "up") {
...
export default function MisServiciosScreen() {
  ...
  useEffect(() => {
    ...
    getDoc(doc(db, "users", user.uid)).then((s) => {
      if (s.exists()) setMyName(s.data()?.displayName || "");
    });
  ...
  const notifyRequester = async (req, type) => {
    try {
      const snap = await getDoc(doc(db, "users", req.fromUserUid));
      ...
      if (type === "accepted") {
        await sendExpoPush(token, {
          ...
          body: `${myName || "Proveedor"} aceptó tu solicitud de ${
            req.serviceName
          }${req.offer ? ` — Oferta: ${fmtCOP(req.offer)} COP` : ""}`,
          data: { type: "request:accepted", ...base },
        });
      } else if (type === "rejected") {
        await sendExpoPush(token, {
          title: "Solicitud rechazada",
          body: `${myName || "Proveedor"} rechazó tu solicitud de ${
            req.serviceName
          }`,
          data: { type: "request:rejected", ...base },
        });
      } else if (type === "finished") {
        await sendExpoPush(token, {
          title: "Servicio finalizado",
          body: `${myName || "Proveedor"} finalizó la solicitud de ${
            req.serviceName
          }`,
          data: { type: "request:finished", ...base },
        });
      }
    } catch (e) {
      console.warn("notifyRequester error:", e?.message || e);
    }
  };
  ...
  const accept = async (item) => {
    try {
      ...
      await notifyRequester(item, "accepted");
      setDetail(null);
    } catch (e) {
        ...
        { text: "No", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              await notifyRequester(item, "rejected");
              setDetail(null);
            } catch (e) {
    ...
    try {
      ...
      await runTransaction(db, async (tx) => {
        ...
        if (req.status !== "aceptada")
          throw new Error("Estado no válido para finalizar");
      ...
      if (!isSelfRequest && targetUidOutside) {
        try {
          await bumpReputation(db, targetUidOutside, value, item.id); // 👈 pasamos requestId (no imprescindible)
        } catch (e) {
          if (e?.code === "permission-denied") {
            Alert.alert(
              "Reputación",
              "No se pudo actualizar la reputación por permisos."
            );
          } else {
            console.warn(
              "bumpReputation falló (no crítico):",
              e?.code || e?.message || e
            );
          }
        }
      ...
      await notifyRequester(item, "finished");
      ...
      Alert.alert(
        "¡Listo!",
        isSelfRequest
          ? "Se finalizó sin calificación (solicitud propia)."
          : "Has finalizado y calificado la solicitud."
      );
    } catch (e) {
      console.error("Error al finalizar (tx):", e);
      Alert.alert(
        "Error",
        e?.code === "permission-denied"
          ? "Permisos insuficientes según las reglas."
  ...
  const clearHistory = async () => {
    try {
      ...
      await batch.commit();
      Alert.alert("Listo", "Se limpió tu historial de solicitudes.");
    } catch (e) {
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
      <View style={styles.container}>
        <Text style={styles.pageTitle}>Solicitudes recibidas</Text>
        ...
        {tab === "historial" && (
          <View style={styles.infoBox} accessibilityRole="summary">
            ...
            <Text style={styles.infoText}>
              Los registros del historial se eliminan automáticamente{" "}
              <Text style={{ fontWeight: "800" }}>cada {TTL_DAYS} días</Text>{" "}
              desde que una solicitud se finaliza, rechaza o cancela.
            </Text>
          </View>
        )}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Empty text="No hay elementos en esta sección." />
          }
            ...
            <View style={styles.modalBox}>
              ...
              {detail ? (
                <>
                  ...
                  <Text style={styles.line}>
                    <Text style={styles.bold}>Teléfono (solicitante):</Text>{" "}
                    {detail.fromPhone || "—"}
                  </Text>
                  <Text style={styles.line}>
                  ...
                  {detail.status === "pendiente" && (
                    <View style={{ flexDirection: "row", marginTop: 10 }}>
                      <TouchableOpacity
                        style={[
                          styles.finishBtn,
                          { flex: 1, backgroundColor: "#198754" },
                        ]}
                        onPress={() => accept(detail)}
                      >
                        <Ionicons
                          ...
                          color="#fff"
                        />
                        <Text style={styles.finishText}> Aceptar</Text>
                      </TouchableOpacity>
                      <View style={{ width: 10 }} />
                      <TouchableOpacity
                        style={[styles.cancelBtn, { flex: 1 }]}
                        onPress={() => reject(detail)}
                      >
                        <Ionicons
...
const styles = StyleSheet.create({
  ...
  // Banner informativo (pastel) — igual que en MisSolicitudesScreen
  infoBox: {
    flexDirection: "row",
    alignItems: "center",
    marginHorizontal: 12,
